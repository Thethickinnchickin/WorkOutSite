<template>
    ...
    <section class="notFound">
        ...
        <div class="text">
        <h1>SORRY DOC</h1>
        <h2 v-if="!$auth.$state.loggedIn" style="font-size: 30px">You Are Not Authorized To Access This Page</h2>
        <h2 v-else>SOMETHING WENT WRONG</h2>
        <div class="row">
            <a style="font-size: 30px" class="col" href="/">BACK TO HOME?</a>            
        </div>
        <div v-if="!$auth.$state.loggedIn" class="row mt-3">
            <a style="font-size: 30px; color: white" class="col" href="/login">SIGN IN?</a>
            <a style="font-size: 30px; color: red" class="col" href="/register">REGISTER?</a>
        </div>
