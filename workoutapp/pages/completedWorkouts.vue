...
<main>
  <div v-if="!loading">
    <h2 class="pb-2 border-bottom mt-5 pt-5"
      style="color: rgb(57, 165, 17); text-align: center;">Completed Workouts</h2>
    <h3 class="mr-3 mt-3 text-center">Page: {{pageNumber}}</h3>        
    <div class="row mt-4 mx-3 ">
        <div v-for="workout in workoutsCompleted" :key="workout._id" class="col-3 mt-3 mr-0">
        ...
        <div @click="routeRedirct('/workout/new')" class="col-3 mr-0 mt-3">
            <div class="our_solution_category mt-3">
                <div class="solution_cards_box">
    ...
    <div class="row mt-5">
      <div class="col">
        <div class="pagination p1 justify-content-center">
          <ul>
            <a @click="pageChange(pageNumber, 'last')"><li>&lt;</li></a>
            <a style="color: white" :class="index === pageNumber ? 'is-active' :''" 
            :disabled="index === pageNumber ? true : false"
            v-for="index in totalPages" :key="index" @click="pageChange(index)"><li>{{index}}</li></a>
            <a @click="pageChange(pageNumber, 'next')"><li>></li></a>
          </ul>
        </div>      
      </div> 
    </div> 
...
export default {
  components: {
    ...
    async created() {
        ...
        this.totalPages = completedWorkoutsresponse.totalPages;
        this.workoutsCompleted = completedWorkoutsresponse.workouts
